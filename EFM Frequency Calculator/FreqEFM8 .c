...
void main (void) 
{
	...
	while(1)
	{
		...
		// F = 1.44 / (3000* overflow_count * 0x10000L + TH0 * 0x100L + TL0);
		capacitance = 1.44 * 1000000 / (3000 * (float)F);
		
		capacitance = 1000* capacitance;
		//getsn(buff, sizeof(buff));
		sprintf(buff,"%.4f",capacitance);
		LCDprint(buff, 2, 1);
		printf("\rFreq = %lu Hz\n", F);
		printf("C = %g nF", capacitance);
